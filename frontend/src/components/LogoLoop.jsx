...
const useAnimationLoop = (
    ...
    }, [targetVelocity, seqWidth, seqHeight, isHovered, hoverSpeed, isVertical, trackRef]); // eslint-disable-line
};
    ...
    const updateDimensions = useCallback(() => {
        ...
        if (isVertical) {
            ...
            if (seqH > 0) {
                ...
                const vp = containerRef.current?.clientHeight ?? parentH ?? seqH;
                setCopyCount(Math.max(ANIMATION_CONFIG.MIN_COPIES, Math.ceil(vp / seqH) + ANIMATION_CONFIG.COPY_HEADROOM));
            }
        } else if (seqW > 0) {
    ...
    const renderLogoItem = useCallback((item, key) => {
        ...
        const inner = item.href
            ? <a className="logoloop__link" href={item.href}
                aria-label={label || "logo link"} target="_blank" rel="noreferrer noopener">{content}</a>
            : content;
